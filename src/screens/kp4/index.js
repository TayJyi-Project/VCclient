...
class Kp4 extends Component {
    ...
    async componentWillMount() {
        try {
            ...
            this.state.index = await AsyncStorage.getItem("@index5") || 1
    ...
    async nextItem() {
        ...
        //if (this.state.index === 3) {
            ...
            result[4].recorded = true
            result[4].trained = true
            await AsyncStorage.setItem("@index5", "1")
            await AsyncStorage.setItem("@preBuild", JSON.stringify(result))
            this.props.navigation.navigate("VoiceList")
        } else {
            this.setState({index: this.state.index + 1})
            await AsyncStorage.setItem("@index5", JSON.stringify(this.state.index))
            await this.setupSound()
            await this.playSound()
        }
    }
